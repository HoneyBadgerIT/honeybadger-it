...
global $wpdb;
$sql=$wpdb->prepare("select * from ".$wpdb->prefix."honeybadger_wc_emails where wc_status='wc-customer-note' and enabled=%d",1);
...
if(isset($result->id))
{
    ...
    $email_heading=str_ireplace("{customer_note}",wpautop( wptexturize( make_clickable( $customer_note ) ) ),$email_heading);
    ...
    $email_subheading=str_ireplace("{customer_note}",wpautop( wptexturize( make_clickable( $customer_note ) ) ),$email_subheading);
    ...
    $additional_content=str_ireplace("{customer_note}",wpautop( wptexturize( make_clickable( $customer_note ) ) ),$additional_content);
}
